...
public:
   ...
   Parser(std::vector<Token>& tokens, std::string& file_data) : cursor(tokens, file_data) {}
...
public:
   ...
   std::shared_ptr<IAST> parse(Parser& parser)
   {
      ...
      PositionRange         position = inner->position.extend_end(parser.cursor.get_current_range());
...
public:
   ...
   std::shared_ptr<IAST> parse(Parser& parser)
   {
      ...
      IAST         *ast_ptr      = new ASTLiteral(literal_token, literal_token.pos);
...
class BlockParserPrefix : public IParserPrefix {
   ...
   std::shared_ptr<IAST> parse(Parser& parser)
   {
      ...
      parser.cursor.expect(TokenType::CloseCurlyBracket, "expected } to close block");
...
public:
   ...
   std::shared_ptr<IAST> parse(Parser& parser, std::shared_ptr<IAST>& left)
   {
      ...
      PositionRange position = left->position.extend_end(right->position);
...
public:
   ...
   std::shared_ptr<IAST> parse(Parser& parser, std::shared_ptr<IAST>& left)
   {
      ...
      return std::shared_ptr<IAST>(new ASTFunctionCall(left, params, left->position.extend_end(parser.cursor.get_current_range())));
   }
};
...
public:
   ...
   std::shared_ptr<IAST> parse(Parser& parser, std::shared_ptr<IAST>& left)
   {
      ...
      PositionRange position = left->position.extend_end(parser.cursor.get_current_range());
...
void IASTVisitor::map_fn_call(ASTFunctionCall& fn_call)
{
